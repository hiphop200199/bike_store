...
class Product
{
    ...
    public function updateItem()
    {
        ...
        $name = strip_tags(trim($_POST['name']));
        $price = intval(strip_tags(trim($_POST['price']))) ;
        $image_source = strip_tags(trim($_POST['image_source']));
        $theme = strip_tags(trim($_POST['theme']));
        $language = strip_tags(trim($_POST['language']));
        $author = strip_tags(trim($_POST['author']));
        $publisher = strip_tags(trim($_POST['publisher']));
        ...
        $update_sql = 'update products set
            ...
            stock = ?
            where id = ? ';
        $statement = $this->conn->prepare($update_sql);
        $statement->execute([$name,$price,$image_source,$theme,$language,$author,$publisher,$published_date,$introduction,$stock,$item_id]);
        if ($statement->rowCount() > 0) {
            echo json_encode(['message' => 'update success', 'redirect' => 'list.php?page=1']);
            exit;
        }
       
        
    }
    public function deleteCheckedItems() 
    {
        function strToInt($item)
        {
            return intval($item);
        }
        $items = explode(',',strip_tags(trim($_POST['id-range'])));//變成陣列        
        $id = array_map('strToInt',$items);
    ...
    public function getExportData()
    {
        ...
        $page_now = isset($_GET['page'])? intval($_GET['page']):null; //從網址列參數索取當下頁面並轉數字       
        $per_page = 10;
        $data_sql = $keyword===null? sprintf('select id,name,price,theme,language,author,publisher,published_date,stock from products limit %d offset %d', $per_page, ($page_now - 1) * $per_page):sprintf('select id,name,price,theme,language,author,publisher,published_date,stock from products where (name like ? or introduction like ?) limit %d offset %d', $per_page, ($page_now - 1) * $per_page); //特定格式組字串，用offset計算起始位置，搭配limit設定要讀取的範圍
        $statement = $this->conn->prepare($data_sql);
        if($keyword===null){
            $statement->execute();
        }else{
